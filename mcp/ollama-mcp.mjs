...
for (let i=0; i<availableTools.tools.length; i++) {
  ...
  availableTools.tools[i] = { type: 'function', function: availableTools.tools[i] };
};
...
async function handleResponse(messages, response) {
  ...
  if (response.message.tool_calls && (response.message.tool_calls.length != 0)) {
    for (const tool of response.message.tool_calls) {
      ...
      try {
        const funcResponse = await client.callTool({name: tool.function.name, arguments: tool.function.arguments});
        for (let i = 0; i< funcResponse.content.length; i++ ){
          messages.push({ role: 'tool', content: funcResponse.content[i].text });
        }
      } catch (e) {
    ...
    return ( handleResponse( messages,
      await ollama.chat({ model: model, messages: messages, tools: availableTools, options: ollamaOptions})
    ));
  } else { 
...
const messages = [
  {role: 'system', content: 'only answer questions about a favorite color by using the response from the favorite_color_tool'},
  {role: 'system', content: 'when asked for a favorite color if you have not called the favorite_color_tool, call it'},
...
for (let i = 0; i< questions.length; i++) {
  ...
  const response = await ollama.chat({ model: model, messages: messages, tools: availableTools, options: ollamaOptions});
